-- Comment: 1.1路由组绑定匹配成功测试
--        : 1.2路由组绑定匹配成功，内部出现错误测试
--        : 1.3检测req.params.参数是否正确
--        : 2.多路由组测试
--        : 3.使用相同的路由组测试，前缀不同
...
describe("single group router middleware", function()
	...
	it("error middleware should work as normal", function()
		...
		userRouter:post("/create/:id", function(req, res, next)
			count = 2
			-- has one param, so this will pass to an error middleware
			next(errorMsg)
		end)
